...
public class CreateOrderTest {
    ...
    //Добавила тест по созданию заказа с авторизацией, в первой версии диплома, забыла добавить этот тест
    @Test
    ...
    public void createOrderWithoutAuthorizationWithWrongHashTest() {
        Ingredients ingredients = orderClient.getIngredient();
        ingredient.add(ingredients.getData().get(0).get_id() + "jkhiujkjnhf8");
        ingredient.add(ingredients.getData().get(1).get_id() + "9876jknklhnj2");
        Response response = OrderClient.createOrderWithoutAuthorization(order);
        response.then().log().all()
                .statusCode(500);
    }
